...
`10*log10(mean(linear values))`. В третьей панели QGIS исходные наблюдения
показаны точками, а линия — центрированная медиана по пяти соседним датам.
Double-logistic аппроксимация Sentinel-2 к радару не применяется.
