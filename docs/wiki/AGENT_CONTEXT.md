...
- Веб-карта имеет только три базовых режима `Карта`, `Спутник`, `Гибрид`; `Карта` использует прежнюю растровую OpenStreetMap, отдельные слои подписей `Карты` и `Гибрида` используют OpenFreeMap/OpenMapTiles с приоритетом `name:ru`, свежий Sentinel-2 автоматически встраивается в два спутниковых режима, а кадастровый WMS НСПД включается/выключается отдельным overlay-чекбоксом.
