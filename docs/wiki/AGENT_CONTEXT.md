...
- При включении `График по полю` для `SP.gpkg` добавляется прозрачный временный
  слой `КОРНИКС: подписи полей SP`: для полей из поставки КОРНИКС — культура,
  дата посева, средняя температура, осадки и ET₀ последнего дня ряда. Поля с
  данными получают яркую бирюзовую заливку и контур; у остальных полей подписи
  не показываются.
