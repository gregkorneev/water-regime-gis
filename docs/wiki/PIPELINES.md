...
Состояние локальной среды отображается в блоке `Среда` и отдается как JSON через `/environment.json`: найденный QGIS, версия, путь скрытого QGIS Python, состояние НСПД-плагина и наличие пользовательских артефактов.
