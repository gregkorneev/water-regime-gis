...
Основной рабочий интерфейс — QGIS. Для чтения документации также есть лёгкий
локальный web-просмотрщик; Docker, база PostGIS и отдельное desktop-приложение
не используются.

## Wiki в браузере

Нужен только Python 3.9+ (без установки пакетов). Из корня репозитория:

```bash
python3 -m docs_site
```

Откройте [http://127.0.0.1:8000](http://127.0.0.1:8000). Просмотрщик показывает
все Markdown-файлы из `docs/`: навигация группируется по папкам, поиск фильтрует
статьи по названию. Новая или переименованная статья появится в уже открытой
вкладке не позднее чем через 10 секунд; перезапуск сервера не нужен. Остановить
...
| `docs_site/` | автономный localhost-просмотрщик всех Markdown-документов из `docs/` |
