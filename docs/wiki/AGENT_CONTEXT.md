...
- Для SP тот же QGIS-диалог показывает вторым графиком ежедневные модельные
  ...
  — относительный индикатор влажности Sentinel-1 по сезонному VV, индивидуально
  нормированный по переменным границам поля для совпадения среднего и дисперсии
  с КОРНИКС на общих датах. Точки
  показывают исходные даты, линия — медиану по пяти соседним наблюдениям.
- Поля `SP 7.5`, `SP 7.8` и `SP 7.9` исключены из интерактивного анализа и
  нормировки по решению пользователя; исходные данные сохраняются без изменений.
