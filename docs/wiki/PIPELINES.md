...
- WMS-слой пригоден для визуального контроля кадастровых границ, но не является векторной геометрией для расчетов. Следующий шаг — получить/выбрать конкретный кадастровый полигон по точке и сохранить его как GeoJSON/GeoPackage.
