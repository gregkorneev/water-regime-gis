...
- QGIS-плагин имеет инструмент `График по полю`: при наведении на KAA/SP-полигон показывает `field_id`, а двойной щелчок открывает временной график NDVI/NDMI/NDWI/MNDWI/NDRE/SAVI из `outputs/reports/field_zonal_means.csv`; все индексы аппроксимируются как однотипные seasonal curves с возможной полкой, ростом до пика и снижением после пика.
